...
function Invoke-SCCMInitialization { 
...
function Invoke-SCCMDeinitialization { 
...
do {
    ...
    if ($null -ne $taskSequenceResult) {
        ...
        Write-Host "`n Opening $strpath report : " -Foregroundcolor $inputcolor -Nonewline 
        Invoke-Item $strPath
